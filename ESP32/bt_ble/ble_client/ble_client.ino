...
//function for writing on bt_serial a string
...
void loop() {
  ...
  if (connected) {
    std::string value = pRemoteCharacteristic->readValue();
    Serial.println(value.c_str());
    std::string value1 = "Scappato";
    if(value1.compare(value.c_str()) == 0){
    Serial.println("Trovato il cane");
    bluetoothPrintLine(myDevice->getServiceUUID().toString().c_str());


    // Set the characteristic's value to be the array of bytes that is actually a string.
    pRemoteCharacteristic->writeValue("Trovato");
    }
  }else if(doScan){
